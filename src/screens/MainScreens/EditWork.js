    ...
    const handleSaveWorkAddress = async () => {
        ...
        .catch( async (error) => {
            ...
            if(errorField === "Unauthorized"){
                ...
                setLoading(false)
                setError(errorField || "Problem editing work address")
                setTimeout(() => {
                    setError(false)
                }, 3000)
            }
            ...
            <Animated.View style={{ transform: [{ translateY: translateYClamped}]}} className="flex-1 flex-col justify-end">
                ...
                <View className={`${props.theme === "dark" ? "bg-[#222831]" : "bg-white"} w-full h-[30%] rounded-t-2xl shadow-2xl items-center`}>
                    ...
                    <View className={`w-full h-[20%] px-3 items-center justify-center flex-row`}>
                        <Text style={{fontSize: fontSize * 1.3}} className={`${props.theme === "dark" ? "text-white" : "text-black"} font-black tracking-tight`}> Edit Work</Text>
                    </View>
